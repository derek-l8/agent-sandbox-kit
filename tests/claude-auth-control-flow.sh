...
# Docker-free control-flow tests for Claude Code authentication-volume cleanup.
...
run_claude_command create_probe false true
HARNESS
...
START_RC=0 PRE_PRUNE_RC= POST_PRUNE_RC=8 \
  run_scenario post_cleanup_failure_visible 1 'Claude Code authentication-volume cleanup failed'

START_RC=37 PRE_PRUNE_RC= POST_PRUNE_RC=8 \
  run_scenario command_failure_wins 37 'Claude Code authentication-volume cleanup failed'

printf 'RESULT: Claude Code authentication control-flow checks passed\n'
