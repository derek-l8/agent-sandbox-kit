...
validate_auth_labels claude probe true claude auth-probe 2 3.2.0
if validate_auth_labels claude probe true claude auth-probe '' 2.0.1 2>/dev/null; then exit 1; fi
...
for spec in \
  ...
  'opencode false opencode auth-probe 2 2.0.2' \
  'claude true codex auth-probe 2 3.2.0' \
  'claude true claude wrong 2 3.2.0' \
  'claude true claude auth-probe 99 3.2.0' \
  'claude false claude auth-probe 2 3.2.0'; do
