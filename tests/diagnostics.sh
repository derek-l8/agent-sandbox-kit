...
grep -q 'source=3.2.0, installed=2.0.1' "$work/mismatch"
...
KIT_VERSION=3.2.0 NETWORK_IMAGE=local/codex-sandbox-networked:3.2.0 OPENCODE_IMAGE=local/codex-sandbox-opencode:3.2.0
...
if check_image_label "$PROJECT_NETWORK_IMAGE" io.codex-sandbox.kit.version 3.2.0 2.0.1 2>"$work/image"; then exit 1; fi
for expected in 'Image: local/codex-sandbox-networked:2.0.1' 'Label: io.codex-sandbox.kit.version' 'Expected: 3.2.0' 'Actual: 2.0.1' 'sbx upgrade stale-probe' 'Safe remediation: sbx build'; do grep -q "$expected" "$work/image"; done
