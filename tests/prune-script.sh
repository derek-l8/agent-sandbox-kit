...
bash "$prune" "$d1" || fail_out "prune failed on a normal directory"
...
echo "PASS: junk removed and auth.json normalized to 0600"
...
bash "$prune" "$d3" || fail_out "prune failed on a junk-only volume"
