...
# Root-owned authentication-volume pruning script, baked into the Claude Code networked images at
...
# credential file (.credentials.json), and fails closed: any deletion failure,
...
if [[ -e "$auth_dir/.credentials.json" || -L "$auth_dir/.credentials.json" ]]; then
  if [[ -f "$auth_dir/.credentials.json" && ! -L "$auth_dir/.credentials.json" ]]; then
    if ! node -e 'JSON.parse(require("fs").readFileSync(process.argv[1], "utf8"))' \
        "$auth_dir/.credentials.json" >/dev/null 2>&1; then
      printf 'ERROR: authentication credential is malformed JSON: %s\n' "$auth_dir/.credentials.json" >&2
      exit 1
    fi
    chmod 0600 "$auth_dir/.credentials.json"
  else
    rm -rf -- "$auth_dir/.credentials.json"
...
# Delete every direct child except .credentials.json. A deletion failure aborts
# through set -e instead of being reported as success.
unexpected="$(find "$auth_dir" -mindepth 1 -maxdepth 1 ! -name .credentials.json -print -quit)"
find "$auth_dir" -mindepth 1 -maxdepth 1 ! -name .credentials.json -exec rm -rf -- {} +
...
# Postcondition: nothing except .credentials.json may remain.
unexpected="$(find "$auth_dir" -mindepth 1 -maxdepth 1 ! -name .credentials.json -print -quit)"
if [[ -n "$unexpected" ]]; then
...
if [[ -e "$auth_dir/.credentials.json" || -L "$auth_dir/.credentials.json" ]]; then
  ...
  mode="$(stat -c '%a' "$auth_dir/.credentials.json")"
  if [[ "$mode" != "600" ]]; then
